...
# Создаем CommentForm прямо в views.py
...
def home(request):
    """Главная страница с каруселью и последними статьями"""
    # Получаем последние 3 статьи для главной страницы
    latest_posts = Blog.objects.all().order_by('-posted')[:3]
    
    return render(request, 'app/index.html', {
        'title': 'Clothes&Fashion - Главная страница',
        'year': datetime.now().year,
        'latest_posts': latest_posts,  # Добавлено для отображения статей на главной
    })
...
def blogpost(request, parametr):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            # Создаем комментарий вручную, так как это не ModelForm
